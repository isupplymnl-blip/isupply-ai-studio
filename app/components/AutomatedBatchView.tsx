  ...
  return (
    <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* Slide navigator */}
        <div style={{
          background: 'var(--studio-surface)', borderBottom: '1px solid var(--studio-border)',
          ...
          <button
            ...
            style={{
              width: 28, height: 28, borderRadius: 6, border: '1px solid var(--studio-border)',
              background: 'var(--studio-elevated)', color: currentIndex === 0 ? 'var(--studio-border)' : 'var(--studio-text-sec)',
              ...
              return (
                <button key={s.id} onClick={() => setCurrentIndex(i)}
                  style={{
                    ...
                    border: `1px solid ${i === currentIndex ? 'var(--studio-accent)' : error ? '#F43F5E44' : hasImage ? '#0D948844' : hasPrompt ? 'var(--studio-border)' : 'var(--studio-elevated)'}`,
                    background: i === currentIndex ? 'var(--studio-accent)' : error ? '#F43F5E11' : hasImage ? '#0D948811' : 'var(--studio-elevated)',
                    color: i === currentIndex ? '#fff' : error ? '#F43F5E' : hasImage ? '#0D9488' : 'var(--studio-text-muted)',
          ...
          <button
            ...
            style={{
              width: 28, height: 28, borderRadius: 6, border: '1px solid var(--studio-border)',
              background: 'var(--studio-elevated)', color: currentIndex === slides.length - 1 ? 'var(--studio-border)' : 'var(--studio-text-sec)',
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--studio-border)', background: 'var(--studio-bg)', flexShrink: 0 }}>
          <div style={{ maxWidth: 680, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{
                  width: 6, height: 6, borderRadius: '50%',
                  background: currentSlide?.isLoading ? '#F43F5E' : 'var(--studio-accent)',
                  boxShadow: `0 0 6px ${currentSlide?.isLoading ? '#F43F5E' : 'var(--studio-accent)'}`,
                  animation: currentSlide?.isLoading ? 'pulse 1s infinite' : 'none',
                }} />
                <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--studio-text)' }}>Slide {currentIndex + 1}</span>
                <span style={{ fontSize: 10, color: 'var(--studio-text-sec)', background: 'var(--studio-elevated)', padding: '2px 8px', borderRadius: 20, border: '1px solid var(--studio-border)' }}>
                  Image Prompt
                </span>
              </div>
              <div style={{ display: 'flex', gap: 5 }}>
                <button onClick={addSlide}
                  style={{ padding: '3px 10px', fontSize: 10, fontWeight: 600, borderRadius: 5, border: '1px solid var(--studio-border)', background: 'var(--studio-elevated)', color: 'var(--studio-text-sec)', cursor: 'pointer' }}>
                  + Add Slide
                </button>
                {slides.length > 1 && (
            ...
            <textarea
              ...
              style={{
                ...
                borderRadius: 7, padding: '8px 10px', color: 'var(--studio-text)', fontSize: 12,
            ...
            {tagStatuses.length > 0 && (
              <div style={{ background: 'var(--studio-surface)', border: '1px solid var(--studio-border)', borderRadius: 7, padding: '8px 10px', marginTop: 8 }}>
                <p style={{ fontSize: 9, color: 'var(--studio-text-muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Reference Detection</p>
                {tagStatuses.map(s => (
                  <div key={s.name} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 11, color: s.matched ? 'var(--studio-text)' : 'var(--studio-text-muted)', flex: 1 }}>{s.name}</span>
        ...
        {/* Generate All button */}
        <div style={{ padding: '12px 20px', background: 'var(--studio-bg)', borderBottom: '1px solid var(--studio-border)', flexShrink: 0 }}>
          <div style={{ maxWidth: 680, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 10 }}>
            <button
              ...
              style={{
                ...
                color: (isGeneratingAll || anyLoading || promptsFilledCount === 0) ? 'var(--studio-text-muted)' : '#fff',
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'var(--studio-surface)', cursor: 'pointer',
        transition: 'border-color 0.15s',
        boxShadow: isActive ? '0 0 0 1px color-mix(in srgb, var(--studio-accent) 27%, transparent)' : 'none',
      }}
    >
      {/* Image area */}
      <div style={{ position: 'relative', aspectRatio: '4/5', background: 'var(--studio-bg)' }}>
        {slide.isLoading && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            <div style={{
              width: 28, height: 28, border: '2px solid var(--studio-border)', borderTopColor: 'var(--studio-accent)',
              borderRadius: '50%', animation: 'spin 1s linear infinite',
            }} />
            <span style={{ fontSize: 10, color: 'var(--studio-text-muted)' }}>Generating…</span>
          </div>
        )}
            ...
            {hovered && (
              <div style={{
                ...
                <button onClick={e => { e.stopPropagation(); onOpen(); }}
                  style={{ padding: '5px 10px', fontSize: 10, fontWeight: 600, borderRadius: 5, border: 'none', background: 'var(--studio-accent)', color: '#fff', cursor: 'pointer' }}>
                  Open
                </button>
                ...
                <a href={slide.imageUrl} download={`slide-${index + 1}-${Date.now()}.png`}
                  ...
                  style={{ padding: '5px 10px', fontSize: 10, fontWeight: 600, borderRadius: 5, border: '1px solid var(--studio-border)', background: 'var(--studio-elevated)', color: 'var(--studio-text-sec)', cursor: 'pointer', textDecoration: 'none' }}>
                  Save
                </a>
              </div>
            )}
        ...
        {!slide.imageUrl && !slide.isLoading && !slide.error && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 6 }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, border: '1px dashed var(--studio-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--studio-border)', fontSize: 18 }}>✦</div>
            <span style={{ fontSize: 9, color: 'var(--studio-border)' }}>Not generated</span>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: '7px 9px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 3 }}>
          <span style={{ fontSize: 10, fontWeight: 600, color: isActive ? '#A78BFA' : 'var(--studio-text-sec)' }}>Slide {index + 1}</span>
          {slide.imageUrl && !slide.isLoading && (
            <button onClick={e => { e.stopPropagation(); onRegen(); }}
        ...
        {slide.prompt && (
          <p style={{ fontSize: 9, color: 'var(--studio-text-muted)', lineHeight: 1.4, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
            {slide.prompt}
          </p>
        )}
      </div>
